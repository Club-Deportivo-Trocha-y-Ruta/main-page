---
title: "Formación Juvenil"
subtitle: "Construyendo los campeones del mañana"
icon: "ph:mountains"
image: "/images/placeholder-program.jpg"
ageRange: "9 a 15 años"
ageMin: 9
ageMax: 15
targetLevel: "formación"
schedule: "Sábados y domingos 6:30 - 9:30 AM"
duration: "3 horas"
location: "Punto de encuentro: Parque Belalcázar, Yumbo"
maxStudents: 25
requirements:
  - "Bicicleta de montaña en buen estado"
  - "Casco certificado"
  - "Guantes"
  - "Sistema de hidratación (botella o camelback)"
  - "Kit básico de reparación (cámara de repuesto, palancas, multitool)"
equipmentNeeded:
  - "Bicicleta de montaña con suspensión delantera"
  - "Casco de ciclismo certificado"
  - "Guantes de ciclismo"
  - "Gafas de protección"
  - "Ropa deportiva adecuada"
  - "Botella de agua o camelback"
  - "Kit de reparación básico"
  - "Protector solar"
objectives:
  - "Perfeccionar técnica de pedaleo y manejo de la bicicleta"
  - "Desarrollar resistencia cardiovascular progresiva"
  - "Aprender mecánica básica de la bicicleta"
  - "Introducir conceptos de nutrición deportiva"
  - "Preparar para primeras competencias departamentales"
  - "Fortalecer valores de disciplina y trabajo en equipo"
methodology: "Entrenamiento estructurado con periodización básica. Combinación de trabajo técnico en circuito, rodadas de fondo progresivas y ejercicios de fuerza con peso corporal. Participación en competencias formativas."
enrollmentOpen: true
monthlyFee: "Consultar"
active: true
order: 2
draft: false
---

El programa de Formación Juvenil está diseñado para jóvenes entre 9 y 15 años que ya dominan las habilidades básicas sobre la bicicleta y quieren dar el siguiente paso. Aquí se construyen las bases técnicas, físicas y mentales para la competencia.

## Estructura del entrenamiento

Las sesiones de fin de semana combinan trabajo técnico en circuitos cerrados con salidas a senderos de dificultad progresiva en los alrededores de Yumbo y el Valle del Cauca. Los sábados se enfocan en técnica y habilidades, mientras que los domingos se dedican a rodadas de mayor distancia.

## Preparación competitiva

Los corredores de este programa participan en competencias departamentales y regionales bajo la supervisión de nuestros entrenadores. Cada participación se planifica como parte del proceso formativo, priorizando el aprendizaje sobre los resultados.

## Formación integral

Además del trabajo en bicicleta, el programa incluye talleres de mecánica básica, charlas de nutrición deportiva y sesiones de fortalecimiento físico. Buscamos formar deportistas completos que entiendan su cuerpo y su herramienta de trabajo.
