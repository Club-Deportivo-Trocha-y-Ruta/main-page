...
14 colecciones definidas; 10 con directorio y contenido: `events`, `faqs`, `gallery`, `news`, `programs`, `riders`, `social-initiatives`, `species` (~32), `sponsors`, `trees` (~77). `results` ya tiene directorio (`src/content/results/`) pero **sin datos todavía**: solo su `README.md`, que documenta el formato y que el loader ignora porque lee `yaml`/`yml`/`json`. Sin archivos, el tablero de la temporada de `/noticias` no se pinta. `pages` no genera rutas: guarda el copy que una página no puede deducir de su propio contenido (hoy solo `programas.md`, con el bloque `agePicker` del selector de edad). `directivos` tiene directorio pero solo su `README.md` (el patrón del loader lo excluye), y `rutas` sigue **sin directorio**: si trabajas con ella, créalo primero.
